...
import { InvoiceItem, PaymentReceipt, SalesInvoice } from '../types';
...
export const SalesView: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTabLocal] = useState<'invoices' | 'returns'>('invoices');

  React.useEffect(() => {
    if (globalSubTab && ['invoices', 'returns'].includes(globalSubTab)) {
      setActiveSubTabLocal(globalSubTab as any);
    }
  }, [globalSubTab]);

  const setActiveSubTab = (tab: 'invoices' | 'returns') => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
        ...
        >
          ...
          مردودات ومسموحات المبيعات (إشعارات دائنة)
        </button>
      </div>
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                إصدار فواتير ضريبية مفصلة مع الرمز المشفر QR، وحساب ضريبة القيمة المضافة ({companyProfile.defaultVatRate}%)، والترحيل المحاسبي الآلي
              </p>
            </div>
          ...
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
                ...
                <th className="py-3 px-4">الإجمالي شامل الضريبة</th>
          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full p-8 shadow-2xl border border-slate-200 max-h-[95vh] overflow-y-auto text-slate-900">
            ...
            <div className="border border-slate-300 p-6 rounded-xl space-y-6">
              ...
              <div className="flex justify-between items-start border-b border-slate-200 pb-4">
                <div>
                  <h1 className="text-xl font-extrabold text-slate-900">{companyProfile.nameAr || 'شركة التقنيات الحديثة للتجارة'}</h1>
                  <p className="text-xs text-slate-500 mt-0.5">{companyProfile.address || 'الرياض - المملكة العربية السعودية'} | هاتف: {companyProfile.phone || '0100000000'}</p>
                  <p className="text-xs font-mono font-bold text-slate-700 mt-1">
                    الرقم الضريبي للمنشأة: {companyProfile.taxNumber || '300123456700003'} | س.ت: {companyProfile.commercialRegister || '1010998877'}
                  </p>
                </div>
